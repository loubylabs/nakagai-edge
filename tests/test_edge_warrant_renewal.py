...
# on. These use the unresolvable market-exit case on purpose: its direction
# and account are both perfectly good, so only `blocked` can catch it.


def _blocked_rec(**over):
    return _rec(blocked="connector cannot express a canonical market exit",
                anomaly="connector cannot express a canonical market exit",
