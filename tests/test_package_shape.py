...
def test_edge_release_version_is_058():
    import tomllib
    ...
    assert project["version"] == "0.5.8"

    lock = (root / "uv.lock").read_text()
    readme = (root / "README.md").read_text()
    assert 'name = "nakagai-edge"\nversion = "0.5.8"' in lock
    assert "0.5.8" in readme
    assert "canonical_order_required" in readme

    ci_path = root / ".github" / "workflows" / "ci.yml"
    if ci_path.exists():
        assert "version('nakagai-edge') == '0.5.8'" in ci_path.read_text()
    else:
